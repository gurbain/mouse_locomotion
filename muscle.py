...
class HillMuscle:
	...
	def __init__(self, scene_, params_):
		...
		self.PEE_K_PEE = self.PEE_F_PEE*( self.CE_F_max/ ( self.CE_l_CEopt*(self.CE_DeltaW_limb_des+1-self.PEE_L_PEE0) ) ** self.PEE_v_PEE )
	...
	def update(self, l_CE, l_MTC, dot_l_MTC, q):
		...
		D0 = self.CE_l_CEopt * B_rel * self.SDE_d_SEmax * (self.SDE_R_SE + (1 - self.SDE_R_SE) * (q * F_isom+F_PEE / self.CE_F_max))
		...
		if dot_l_CE > 0:
			...
			if (C1 ** 2 - 4 * C2 * C0) < 0:
				...
				dot_l_CE = (- C1 + math.sqrt(C1 ** 2 - 4 * C2 * C0)) / (2 * C2) # note that here +sqrt gives the correct solution
		...
		F_SDE = self.SDE_d_SEmax * ((1-self.SDE_R_SE) * ((F_CE+F_PEE) / self.CE_F_max) + self.SDE_R_SE) * (dot_l_MTC - dot_l_CE)
		F_MTC = F_SEE + F_SDE
